...
subscriberListen(UA_PubSubChannel *psc) {
    ...
    if(rv == UA_STATUSCODE_GOODNONCRITICALTIMEOUT || rv == UA_STATUSCODE_GOODNODATA) {
...
VOID  thread_server_entry(ULONG thread_input)
{
    ...
    UA_PubSubTransportLayer ethLayer = UA_PubSubTransportLayerEthernet();
    ...
    UA_PubSubChannel *psc =
        ethLayer.createPubSubChannel(&connectionConfig);
